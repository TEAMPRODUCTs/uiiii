...
function replaceToLowerCase(fileDir,type){
	...
	if(tp!=fileDir){
		console.log("[ replace "+type+"] "+fileDir+" ==>"+tp);
	}
	return tp;
}
//配置环境
...
(function(){
	...
	function createLocalRoute(){
			...
			if (urlSchema){
				...
				for(var i=0;i<urlSchema.length;i++){
					...
					*/
					...
					// };
					console.log('[ Route ]'+' '+us+' : '+map[oUrl].filePath);
				}
		...
		*/
		...
		fs.writeFileSync(replaceToLowerCase(path.resolve(config.absRoot,'LizardLocalroute.js')),content);
	}
...
function fetchFile(oUrl,tIns,callback){
	...
	if (/*!specFlag.exclude && */(/*specFlag.include ||  */isView ||isInResourcePaths(oUrlObj)/*oUrlObj.host in config.resource*/)){
		...
		*/
		...
		// if(hostname){
			// filePath=path.resolve(root,"../",(hostMaps[hostname]?hostMaps[hostname]:hostname),oUrlObj.pathname.replace(/^\//,'').replace(/[^\w\.\/\\]/g,'_'))+(isView?'.html':'');
		...
		if(hostname1){
			 filePath=path.resolve(root,"../",(hostMaps[hostname1]?hostMaps[hostname1]:hostname1),oUrlObj.pathname.replace(/^\//,'').replace(/[^\w\.\/\\]/g,'_'))+(isView?'.html':'');
			 fileDir=path.dirname(filePath);
		...
		var opts=url.parse((tUrl.indexOf('web/lizard.seed.js')!=-1)?tUrl.replace('web/lizard.seed.js','hybrid/lizard.seed.js'):tUrl);
